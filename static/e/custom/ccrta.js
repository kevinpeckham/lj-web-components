!function(){
	...
	function customStyling(){let pageId='';const pageTemplateClass=evaluateBodyClass();if(!pageTemplateClass)return;const hasLJ=document.documentElement.classList.contains('lj');if(hasLJ)return;addCSP();scrubScripts();scrubStyles();const newBodyContent=createNewBodyContent();replaceBodyContent(newBodyContent);const html=document.documentElement;html.classList.add('lj');function evaluateBodyClass(){const approvedPageClasses=['page-template-page-dbe-home','page-template-page-dbe'];const bodyClasses=document.body.className.split(' ');const templateClass=approvedPageClasses.find((cls)=>bodyClasses.includes(cls));const pageIdClass=bodyClasses.find((cls)=>cls.includes('page-id-'));pageId=pageIdClass?pageIdClass.replace('page-id-',''):'';if(!templateClass)return'';document.body.className=`${templateClass} ${pageIdClass}`;document.body.dataset.pageId=pageId;return templateClass}
	function addCSP(){const existingCSP=document.getElementById('csp');const csp=`<meta id="csp" http-equiv="Content-Security-Policy" content="default-src 'self'; img-src https://res.cloudinary.com; style-src https://lj.cdn.dev; script-src https://lj.cdn.dev; font-src 'self' https://fonts.googleapis.com; connect-src 'self'; frame-src 'self'; child-src 'self'; object-src 'none'; form-action 'self'; base-uri 'self'; block-all-mixed-content; upgrade-insecure-requests;">`;if(!existingCSP)document.head.insertAdjacentHTML('afterbegin',csp);if(existingCSP)existingCSP.outerHTML=csp}
	function scrubScripts(){const headScripts=document.head.querySelectorAll('script');headScripts.forEach((script)=>script.remove())}
	function scrubStyles(){const headStyleLinks=Array.from(document.head.querySelectorAll('[rel=stylesheet]'));headStyleLinks.forEach((link)=>{if(!link.href.includes('cdn.lj.dev')){link.remove()}});const headStylesElements=Array.from(document.querySelectorAll('style'));headStylesElements.forEach((style)=>{if(!style.hasAttribute('data-vite-dev-id')){style.remove()}})}
	function buildAlertBar(){const template=document.querySelector('template#template-alert');const content=template?.content?(template.content.textContent??''):'';const alertText=content?.replace(/More/,'').trim()??'';const newAlertBarHTML=`
			<div id="alert-bar" class="page-x-padding"><a href="/rider-alert" title="learn more"><span class="alert-content">${alertText}...</span> <span class="alert-cta">Learn More</span></a></div>`;return newAlertBarHTML}
	function formatSiteHeader(){const newHeaderHTML=getNewHeaderHTML();return newHeaderHTML}
	function getBreadCrumb(){const bodyClass=document.body.className;const breadcrumb=bodyClass.includes('page-template-page-dbe')&&!bodyClass.includes('page-template-page-dbe-home')?'DBE & SBE Programs':'';return breadcrumb}
	function getPageTitle(){const titleTemplate=document.querySelector('template#template-page-title');const titleContent=titleTemplate?.content??'';const title=titleContent?(titleContent.textContent??''):'';return title}
	function buildMainInner(){const breadcrumb=getBreadCrumb();const pageTitle=getPageTitle();const mainTemplate=document.querySelector('template#template-main');const mainContent=mainTemplate?.content;const mainInner=document.createElement('div');mainInner.append(mainContent?.cloneNode(!0)??'');mainInner?.insertAdjacentHTML('afterbegin',`<h1>${pageTitle}</h1>`);if(breadcrumb)
	mainInner?.insertAdjacentHTML('afterbegin',`<a id="breadcrumb" href="/dbe-sbe-programs" title="back to DBE & SBE Programs landing page" >${breadcrumb}</a>`);const mainInnerChildren=Array.from(mainInner?.children??[]);mainInnerChildren.forEach((child)=>{child.removeAttribute('class');child.removeAttribute('style')});const nav=mainInner?.querySelector('.dbe-menu');nav?.remove();const mainInnerHTML=mainInner?.innerHTML;return mainInnerHTML}
	function buildSidebarNav(){const sidebarNavTemplate=document.querySelector('template#template-sidebar-nav');const sidebarNavContent=sidebarNavTemplate?.content;const sidebarNavArr=Array.from(sidebarNavContent?.children??[]);const sidebarNavLinksHTML=sidebarNavArr.map((linkParent)=>{if(!linkParent||!(linkParent instanceof HTMLDivElement))return'';const link=linkParent.firstElementChild instanceof HTMLAnchorElement?linkParent.firstElementChild:null;if(!link)return'';const parentClasses=link.parentElement?.className.split(' ');const pageItem=parentClasses?.find((i)=>i.includes('page-item-'));link.className='sidebar-nav-item';if(pageItem?.includes(pageId))link.className+=' active';link.title='go to '+link.textContent;link.textContent=link?.textContent?.replace(/([a-z])\/([a-z])/i,'$1 / $2')?.replace('and','&')?.replace('Resource Links','Resources')??'';return link.outerHTML}).join('');const sidebarNavHTML=`
	...
	function formatLandingNav(sidebarNavHTML){if(!pageTemplateClass.includes('page-template-page-dbe-home'))return'';const classesUpdated=sidebarNavHTML.replaceAll(/sidebar-nav/gi,'landing-nav');const tempDiv=document.createElement('div');tempDiv.innerHTML=classesUpdated;const landingNav=tempDiv.firstElementChild;landingNav?.removeAttribute('class');const label=document.createElement('div');label.className='landing-nav-heading';label.textContent='DBE & SBE Topics';landingNav?.insertBefore(label,landingNav?.firstElementChild);landingNav?.querySelector('.landing-nav-heading')?.remove();return landingNav?.outerHTML??''}
	function formatFooter(){const newFooterHTML=`
		<footer id="footer">
