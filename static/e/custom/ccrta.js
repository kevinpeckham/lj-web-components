!function(){
    ...
    // log raw htm console.log('raw html head',
    // JSON.stringify(document.head.outerHTML)); console.log('raw html body
    // contents', JSON.stringify(document.body.outerHTML)); prevent re-running
    // function -- return if lj class is already present
    const hasLJ = document
        .documentElement
        .classList
        .contains('lj');
    if (hasLJ)
    ...
    function evaluateBodyClass() {
        ...
        // scrub body classes to just the key page class document.body.className =
        // bodyClass;
    ...
    function addCSP() {
        ...
        document
            .head
            .insertAdjacentHTML('afterbegin', csp);
    }
    ...
    function scrubStyles() {
        // remove all stylesheets links from the head of the page, except for the one
        // with href="https://cdn.lj.dev/e/custom/ccrta.css"
        ...
        // remove all style elements from the page except for the ones with
        // data-vite-dev-id
    ...
    function formatAlertBar() {
        const alertText = document.getElementById('head-topbar')
            ?.innerText
                ?
                    .replace(/More/, '')
                    .trim() ?? '';
    ...
    function formatMain() {
        ...
        const breadcrumb = bodyClass.includes('page-template-page-dbe') && !bodyClass.includes('page-template-page-dbe-home')
            ? 'DBE & SBE Programs'
            : '';

        const h1 = document.querySelector('h1');
        const h1Text = h1
            ? JSON.parse(JSON.stringify(h1
                ?.textContent
                    ?.trim()))
            : '';
        ...
        const main = defaultContent
            ?.querySelector('main');
        const mainInner = main
            ?.firstElementChild;

        // insert new h1 into main inner
        mainInner
            ?.insertAdjacentHTML('afterbegin', `<h1>${h1Text}</h1>`);
        if (breadcrumb)
            mainInner
                ?.insertAdjacentHTML('afterbegin', `<a id="breadcrumb" href="/dbe-sbe-programs" title="back to DBE & SBE Programs landing page" >${breadcrumb}</a>`);

        // scrub inline classes and styles from all children of main inner
        const mainInnerChildren = Array.from(mainInner
            ?.children ?? []);
        ...
        // remove nav
        const nav = mainInner
            ?.querySelector('.dbe-menu');
        nav
            ?.remove();

        const mainInnerHTML = mainInner
            ?.innerHTML;
    ...
    function formatSidebarNav() {
        ...
        window
            .console
            .log('sidebarNav', sidebarNav);

        /** @type {HTMLElement[]} */
        const sidebarNavLinksArr = Array.from(sidebarNav
            ?.querySelectorAll('.side-nav-item') ?? []);

        /** @param { HTMLElement } link */
        const sidebarNavLinksHTML = sidebarNavLinksArr.map((link) => {
    ...
    function formatLandingNav(/** @type { string } */
    sidebarNavHTML) {
        // if not the landing page
        if (!bodyClass.includes('page-template-page-dbe-home'))
            return '';
        ...
        landingNav
            ?.insertBefore(label, landingNav
                ?.firstElementChild);

        // remove heading;
        landingNav
            ?.querySelector('.landing-nav-heading')
                ?.remove();
    ...
    function replaceBodyContent(/** @type {string} */
    newBodyContent) {
        // empty body
        ...
        document
            .body
            .insertAdjacentHTML('afterbegin', '<div id="body-container"></div>');
